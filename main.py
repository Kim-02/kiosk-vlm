...
DETECT_PROMPT = (
    "이미지들은 실제 작업 현장 CCTV 연속 프레임이다.\n"
    "판단할 위험 행동은 '라바콘 접촉(cone_touch)' 단 하나뿐이다.\n\n"
    "cone_touch (라바콘 접촉): 사람의 손(또는 신체)이 주황색 라바콘에 실제로 닿아 있거나 잡고 있는 경우.\n"
    "   - 라바콘을 손으로 만지거나 잡거나 들어 올리거나 발로 차는 등 명확한 접촉이 있어야 한다.\n"
    "   - 라바콘 근처를 지나가거나 옆에 서 있기만 하면 접촉이 아니다(정상).\n"
    "   - 접촉이 프레임에서 명확히 보일 때만 위험으로 판단하라. 추측하지 마라.\n\n"
    "연속 프레임에서 라바콘 접촉이 실제로 발생했을 때만 detections에 넣어라.\n"
    "발생하지 않았으면 detections를 빈 배열로 둬라.\n"
    "[출력 형식] 아래 JSON 객체 하나만 출력하라. 마크다운/코드펜스/추가설명 금지.\n"
    '{"detections": [{"label": "cone_touch", "confidence": 0.0~1.0, "evidence": "관찰 근거 한 문장"}]}\n'
    "확실하지 않으면 detections에 넣지 마라."
)
