...
**먼저 "사람이 있는가?"(`Is there any person?`)를 단일 추론으로 한 번 확인합니다.** 모델이 `true`로 답할 때만 사람이 있는 것으로 보고(`false`/판정불가는 사람 없음) 라벨 판정 단계로 넘어갑니다. 사람이 없으면 라벨 배치를 돌리지 않고 곧바로 "안전 이상 없음"으로 끝냅니다(사람 없음 = 안전 이상 없음, 동일 상태).

사람이 있을 때만, 각 라벨은 라벨별 질문(`LABEL_CRITERIA`)을 모델에 던져 `true/false` 답을 받고, 라벨마다 정의된 위반 기준(`VIOLATION_WHEN`)으로 위반 여부를 판정합니다.

| 라벨 | 모델에 묻는 질문 | 위반 조건 | TTS 문구 |
|------|------------------|:---------:|----------|
| `helmet_off` | 모든 작업자가 안전모를 썼는가? | `false`(미착용) | 안전모를 착용하세요. |
...
프레임 폴더 경로만 전달하면, **① 사람 존재 여부를 1회 추론으로 확인**한 뒤 사람이 있으면 **② 라벨을 한 번의 배치 추론**으로 판정합니다. 사람이 없으면 ②를 건너뛰고 위반 없음으로 응답합니다.
