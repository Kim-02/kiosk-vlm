...
각 라벨은 라벨별 질문(`LABEL_CRITERIA`)을 모델에 던져 `true/false` 답을 받고, 라벨마다 정의된 위반 기준(`VIOLATION_WHEN`)으로 위반 여부를 판정합니다.

위반으로 판정된 라벨이 하나도 없으면 "안전 이상 없음"으로 처리됩니다. **사람이 없는 장면은 모든 라벨이 정상으로 판정되어 위반이 비므로, "안전 이상 없음"과 동일한 상태로 취급됩니다**(별도의 사람 검출 단계는 없음).
